...
(function ($) {
    ...
    $.fn.modal = function (options) {
    	var modalimplinit =$.modal.impl.init(this, options) ;
    	contentsUI(); // function in global_obank 20141031
        return modalimplinit;
    };
    $.modal.defaults = {
        appendTo: 'body',
        focus: true,
        ...
        zIndex: 1000,
        close: true,
        closeHTML: '<a class="modalCloseImg" href="#none"><em class="hidden">닫기</em></a>',
    ...
    $.modal.impl = {
        ...
        create: function (data) {
            ...
            s.d.container = $('<div></div>').attr({'id': s.o.containerId , 'tabindex' : 0}).addClass('simplemodal-container').css($.extend(s.o.containerCss, {
                display: 'none',
                position: 'fixed',
                zIndex: s.o.zIndex + 2
//	        })).append(s.o.close && s.o.closeHTML ? $(s.o.closeHTML).addClass(s.o.closeClass) : '').appendTo(s.o.appendTo);
            })).appendTo(s.o.appendTo);
            s.d.wrap = $('<div></div>').addClass('simplemodal-wrap').css({
                height: 'auto',
                width: 'auto'
            ...
            s.d.data = data.attr('id', data.attr('id') || s.o.dataId).addClass('simplemodal-data').css($.extend(s.o.dataCss, {
                display: 'none'
//            })).appendTo('body');
            })).appendTo('body').append(s.o.close && s.o.closeHTML ? $(s.o.closeHTML).addClass(s.o.closeClass) : '');
            data = null;
        ...
        bindEvents: function () {
            ...
            $(document).bind('keydown.simplemodal', function (e) {
                if (s.o.focus && e.keyCode == 9) {
                    s.watchTab(e);
                } else if ((s.o.close && s.o.escClose) && e.keyCode == 27 ) {//&& e.keyCode == 27 || && e.keyCode == 13  13 추가시 확인 버튼에서 닫기된다고함
                    e.preventDefault();
                    s.close();
                }
            });
        ...
        focus: function (pos) {
            var s = this,
                p = pos || 'first';
            var inputEl = ':input:enabled:visible:'+p+', select:enabled:visible:'+p+', a:visible:'+p;
            var input = $( inputEl , s.d.wrap);
            p == 'first' ? input[0].focus() : input[input.length-1].focus();
        },
        ...
        watchTab: function (e) {
            var s = this;
            var $cont = $(e.target).parents('.simplemodal-container');
            pos = e.shiftKey ? 'last' : 'first' ;
            if ($cont.length > 0) {
                s.inputs = $(':input:enabled:visible:first, :input:enabled:visible:last, select:enabled:visible:first, select:enabled:visible:last, a:visible:first, a:visible:last', $cont);
        ...
        open: function () {
            ...
            $(s.d.container).focus();
            s.bindEvents();
            var closeText = $( 'h4:first', $(s.d.container) );
            $( '.'+s.o.closeClass, $(s.d.container) ).attr('href','#none').html( "<em class='hidden'>" + closeText.text() + "(레이어팝업) 닫기</em>" );
        },
        close: function () {
            var s = this;
            if (!s.d.data) {
...
//                		returnBntFocus.focus();
                ...
                // else {
//            		alert("test msg (returnBntFocus = ["+returnBntFocus+"]\n");
//            		$('#wrap').focus();
            	// }
                $("html").removeClass("layerPop_open");
            }
        }
    };
})(jQuery);